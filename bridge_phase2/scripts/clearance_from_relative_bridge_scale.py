...
def main():
    ...
    ap.add_argument("--z_ref_m", type=float, required=True,
                    help="Trial depth at the reference frame")
    ap.add_argument("--use_smoothing", action="store_true",
                    help="Use smoothed relative depth proxy from underside length")
    ...
    for rec in records:
        if is_valid_record(rec):
            valid_idx.append(int(rec["frame_idx"]))
            under_lens.append(underside_length(rec))
        else:
            under_lens.append(None)
    ...
    for rec in records:
        if int(rec["frame_idx"]) == args.ref_frame and is_valid_record(rec):
            ref_rec = rec
            ref_len = underside_length(rec)
            break

    if ref_rec is None or ref_len is None or ref_len <= 1e-9:
        raise RuntimeError(f"Reference frame {args.ref_frame} is missing or invalid.")

    # Build raw depth proxy = ref_len / current_len
    raw_depth_proxy = []
    for rec in records:
        if is_valid_record(rec):
            curr_len = underside_length(rec)
            raw_depth_proxy.append(ref_len / curr_len if curr_len and curr_len > 1e-9 else None)
        else:
    ...
    print("\nBRIDGE-SCALE CLEARANCE PROTOTYPE\n")
    print("frame | time | under_len | rel_depth | Z_t | clearance | status")
    print("-" * 90)
    ...
    for i, rec in enumerate(records):
        ...
        if not is_valid_record(rec):
            ...
            rec_out["clearance_debug"] = {
                "method": "relative_bridge_scale_from_underside_len",
        ...
        z_t = float(args.z_ref_m * rel_depth)
